...
function test_serverbusy()
{
   ...
   if (serveravailable==0) {
      ...
      try { for (i=0;i<nwaterfalls;i++) waterfallapplet[i].destroy(); } catch (e) {} ;
      try { soundapplet.destroy(); } catch (e) {};
      document.body.innerHTML="Sorry, der WebSDR Server ist zu beschäftigt; Bitte versuche es später.\n";
   }
}
...
function bodyonload()
{
   ...
   if (!x && p) p.innerHTML="<b><i>Bitte trage einen Alias/Rufzeichen ein  <a href='#please'>obern in der Seite</a> um bei deiner Unterhaltung dich identifizieren zu können.</i></b>";
...
function setusernamecookie() {
   ...
   if (p) p.innerHTML="Dein Name/Rufzeichen: ";
...
function document_username() 
{
  ...
  if (x) {
    ...
    document.write('<a id="please"><span id="please1"><b><i>Bitte melde dich an, indem Du hier Deinen Namen oder Dein Rufzeichen eingibst (dies wird für spätere Besuche in einem Cookie gespeichert):<\/i><\/b></span> ');
    document.write('<input type="text" value="" name="username" onchange="setusernamecookie()" onclick=""></a>');
  }
}
